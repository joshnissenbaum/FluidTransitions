...
import { View, StyleSheet, UIManager, Easing, InteractionManager, Animated, findNodeHandle } from "react-native";
...
export default class TransitionItemsView extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <View {...this.props} style={[styles.container, style]} ref={ref => (this._viewRef = ref)} collapsable={false}>
        <ScreenContainer style={{ ...StyleSheet.absoluteFillObject }}>{children}</ScreenContainer>
  ...
  getIsAnchored = (name: string, route: string) => {
    ...
    if (
      (sharedElements && sharedElements.find(p => p.fromItem.name === item.anchor && p.fromItem.route === route)) ||
  ...
  getItemByNameAndRoute = (name: string, route: string) => this._transitionItems.getItemByNameAndRoute(name, route);
  ...
  async measureItems(sharedElements: Map<TransitionItem, TransitionItem>, transitionElements: Array<TransitionItem>) {
    const promises = [];
