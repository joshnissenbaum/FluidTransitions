...
class FluidTransitioner extends React.Component<*> {
  ...
  renderTransitionView = (props, prevProps) => {
    ...
    const handlers = this.getPanResponderHandlers(position, index, scene, layout, navigation, props);
    ...
    this._panResponder = !gesturesEnabled
      ...
      : PanResponder.create({
          ...
          onMoveShouldSetPanResponder: (event, gesture) => {
            ...
            const { gestureResponseDistance: userGestureResponseDistance = {} } = options;
          ...
          onPanResponderMove: (event, gesture) => {
            ...
            const axisDistance = isVertical ? layout.height.__getValue() * 0.75 : layout.width.__getValue();
            const currentValue =
  ...
  _renderScene(transitionProps) {
    ...
    return (
      <TransitionRouteView
        ...
        sceneKey={scene.key}
      >
        <SceneView navigation={navigation} screenProps={screenProps} component={scene.descriptor.getComponent()} />
      </TransitionRouteView>
    );
  }
